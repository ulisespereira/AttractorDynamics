...
class TransferFunction:
	...
	def dist_rates(self,x):
		sigma=np.sqrt(1)*self.b # standard normal random variable passed thorugh transfer function
		mu=self.b*(0.-self.h0)
		pdf=(1./np.sqrt(2 * np.pi * sigma**2))*np.exp(-(1./2.)*((np.log(x/(self.rm-x))-mu)/sigma)**2)*(self.rm/(x*(self.rm-x)))
		return pdf
	#TF
	def TF(self,h):
		# sigmoidal TF
