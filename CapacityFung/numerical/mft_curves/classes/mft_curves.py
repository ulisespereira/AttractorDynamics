...
class MFTCurves:
	...
	def overlap_static_MFT(self,alpha,q_init=10.,return_m=True):
		...
		print 'alpha=',alpha,'overlap=',m
		return np.array([(m,q),del0])

	
	#computing capacity static mft
	def capacity_static_MFT(self,init):
		self.del0_c,self.m_c = self.critical.critical_point(init)
		alpha_c = self.del0_c/(self.GI.myLR.gamma * self.GI.Int_TF2(self.del0_c,self.m_c))
		return alpha_c
