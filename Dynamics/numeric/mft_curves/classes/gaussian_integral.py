...
class GaussianIntegrals:
	...
	def Int_TF2(self,del0,m):
		...
		return sol
	# integral TF**2 del1=del0 static MFT
	def Int_DTF2(self,del0,m):
		meanfield = self.myLR.Amp*self.f_eta*m
		sdfield = np.sqrt(del0)*self.xgrid
		dtf = self.myTF.DTF(np.add.outer(meanfield,sdfield))
		sol=self.dx*self.dx*np.einsum('i,ij,ij,j',self.normal_pdf,dtf,dtf,self.normal_pdf)
		return sol
